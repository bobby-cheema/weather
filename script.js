...
const get5days = async(long, lat) => {
    if (long && lat) {
        ...
        const response = await fetch(
            `https://api.openweathermap.org/data/2.5/onecall?lat=${long}&lon=${lat}&exclude=current,minutely,hourly,alerts&units=metric&appid=${API_key}`, { cache: "force-cache" }
        );
        if (response.status === 200) {
            const data = await response.json();
            if (data) {
...
const getonce = async(lon, lat, city) => {
    if (lat && lon) {
        ...
        const response = await fetch(
            `https://api.openweathermap.org/data/2.5/onecall?lat=${lon}&lon=${lat}&exclude=daily,minutely,hourly,alerts&units=metric&appid=${API_key}`, { cache: "force-cache" }
        );
        if (response.status === 200) {
            const data = await response.json();
            if (data) {
...
btn1EL.on("click", async(e) => {
    ...
    const { lat, lon } = await getcoords("Sydney");
    console.log(lat, lon);
...
btn2EL.on("click", async(e) => {
    ...
    if (singledata) {
        ...
        const singledate = new Date(singledata.current.dt * 1000)
            .toLocaleString()
            .substring(1, 10);
        cityEL.html(`<h2 class="d-inline">Melbourne </h6>`)
        dateEL.html(`<h2 class="d-inline"> (${singledate})</h2>`);
        tempEL.html(`<h6 class="mt-3"> Temp:${temprature} C </h6>`);
        windEL.html(`<h6> Wind:${singledata.current.wind_speed} Kmph </h6>`);
        humidityEL.html(`<h6> Humidity:${singledata.current.humidity} % </h6>`);
